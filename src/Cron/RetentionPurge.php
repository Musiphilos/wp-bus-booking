 ...
 *
 *   cutoff = event_end_date + booking_retention_days  (Europe/Lisbon)
 *
 * Once "now" (Lisbon) crosses the cutoff every booking is eligible for deletion.
...
final class RetentionPurge {
	 ...
	 *
	 * @return array{cutoff:string,now:string,window_active:bool,considered:int,deleted:int,skipped_admin:int,dry_run:bool,override_cutoff:bool}
	 */
	public static function sweep( bool $dryRun, ?\DateTimeImmutable $overrideCutoff = null ): array {
		...
		if ( ! $cutoff ) {
			...
			return self::emptyResult( '', $now, false, $dryRun, (bool) $overrideCutoff );
		}
		...
		if ( $now < $cutoff ) {
			return self::emptyResult( $cutoffStr, $now, false, $dryRun, (bool) $overrideCutoff );
		}
	...
	private static function defaultCutoff(): ?\DateTimeImmutable {
		...
		try {
			if ( $endStr !== '' ) {
				$end = new \DateTimeImmutable( $endStr . ' 23:59:59', Time::zone() );
			} elseif ( $startStr !== '' ) {
				// Fallback: assume the event runs 4 days (per spec §4.1).
				$end = ( new \DateTimeImmutable( $startStr . ' 23:59:59', Time::zone() ) )->modify( '+4 days' );
			} else {
				return null;
			}
		} catch ( \Throwable $e ) {
